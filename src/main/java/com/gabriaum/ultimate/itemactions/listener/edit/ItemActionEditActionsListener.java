...
public class ItemActionEditActionsListener implements Listener {
    private final ItemActionsUpdateParamService updateParamService = new ItemActionsUpdateParamService();

    @EventHandler(priority = EventPriority.LOW)
    public void onAsyncPlayerChat(AsyncPlayerChatEvent event) {
        Player player = event.getPlayer();
        if (player.hasMetadata("ultimate_item_actions_edit_actions")) {
            String key = player.getMetadata("ultimate_item_actions_edit_actions").get(0).asString();
            UltimateItemActions ultimateItemActions = UltimateItemActionsRegistry.getByKey(key);
            if (ultimateItemActions == null) {
                player.removeMetadata("ultimate_item_actions_edit_actions", UltimateItemActionsMain.getInstance());
                return;
            }
